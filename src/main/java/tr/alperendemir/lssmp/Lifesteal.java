...
import tr.alperendemir.lssmp.commands.eliminate.EliminateCommand;
import tr.alperendemir.lssmp.commands.health.HealthCommand;
import tr.alperendemir.lssmp.commands.recipe.RecipeCommand;
import tr.alperendemir.lssmp.commands.reset.ResetCommand;
import tr.alperendemir.lssmp.commands.revive.ReviveCommand;
import tr.alperendemir.lssmp.commands.setup.SetupCommand;
import tr.alperendemir.lssmp.commands.withdraw.WithdrawCommand;
import tr.alperendemir.lssmp.configuration.data.eliminations.EliminationConfiguration;
import tr.alperendemir.lssmp.configuration.data.eliminations.EliminationConfigurations;
import tr.alperendemir.lssmp.configuration.data.items.custom.HeartItemConfiguration;
...
import tr.alperendemir.lssmp.configuration.parsers.eliminations.EliminationConfigurationParser;
import tr.alperendemir.lssmp.configuration.parsers.items.custom.HeartItemParser;
import tr.alperendemir.lssmp.configuration.parsers.items.custom.ReviveBeaconParser;
import tr.alperendemir.lssmp.elimination.EliminationManager;
import tr.alperendemir.lssmp.elimination.modes.BanEliminationMode;
import tr.alperendemir.lssmp.feature.heart.*;
import tr.alperendemir.lssmp.items.BaconItem;
import tr.alperendemir.lssmp.items.HeartItem;
import tr.alperendemir.lssmp.items.ReviveBeaconItem;
import tr.alperendemir.lssmp.listeners.FirstJoinListener;
import tr.alperendemir.lssmp.listeners.LifestealCombatListener;
import tr.alperendemir.lssmp.listeners.PlayerChatListener;
import tr.alperendemir.lssmp.pipeline.heart.HeartPipeline;
import tr.alperendemir.lssmp.screens.RecipeScreen;
import tr.alperendemir.lssmp.screens.ReviveScreen;
...
public class Lifesteal {
    ...
    public void onEnable(LifestealInit init) {
        ...
        final var metrics = new Metrics(init, 31114);
    ...
    private void loadItems(LifestealInit init) {
        ...
        var screens = Helix.screens();
        if (screens instanceof tr.alperendemir.helix.screens.HelixScreenRegistryImpl registryImpl) {
            registryImpl.unregisterAll(initPluginName);
        }
    ...
    private void addCustomItemRecipe(LinkedHashMap<String, RecipeConfiguration> recipes, UniqueIdentifier key, ItemDisplayData displayData, RecipeConfiguration recipeConfiguration, String id) {
        ...
        if (defaultStack == null || defaultStack.getType().isAir()) {
            HelixLogger.error("Unable to register recipe '%s' because its item stack could not be created.", id);
            return;
        }

        var namespacedKey = NamespacedKey.fromString(key.toString());
        if (namespacedKey == null) {
            HelixLogger.error("Unable to register recipe '%s' because its key is invalid.", id);
            return;
        }

        Bukkit.removeRecipe(namespacedKey);
        var recipe = recipeConfiguration.createRecipe(namespacedKey, defaultStack);
