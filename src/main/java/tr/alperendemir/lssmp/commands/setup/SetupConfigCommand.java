package tr.alperendemir.lssmp.commands.setup;
...
import tr.alperendemir.lssmp.commands.setup.handlers.SetupBooleanHandler;
import tr.alperendemir.lssmp.commands.setup.handlers.SetupChatHandler;
import tr.alperendemir.lssmp.commands.setup.handlers.SetupCommandHandler;
import tr.alperendemir.lssmp.commands.setup.handlers.SetupEnumHandler;
import tr.alperendemir.lssmp.configuration.data.types.HeartLossMode;
import tr.alperendemir.lssmp.configuration.data.types.PlayerDropHeartsMode;
...
import static tr.alperendemir.helix.logging.LoggerColoring.chatColor;
    ...
    public SetupConfigCommand(Map<UUID, BiConsumer<Player, String>> commandNameGetters, Configuration commandConfig, Configuration generalConfig) {
        ...
        this.handlers.put("complete", new SetupCommandHandler() {
            ...
            public void onSubmit(Player player) {
                ...
                var helixPlugin = HelixPlugin.getProvidingPlugin(SetupConfigCommand.class);
                var loader = Helix.pluginLoader();
                if (loader.getPlugins().containsKey(helixPlugin.getId())) {
                    try {
                        loader.reload(helixPlugin);
                    ...
                    if (bukkitPlugin instanceof tr.alperendemir.lssmp.LifestealInit lifestealInit) {
                        lifestealInit.reloadConfig();
                    } else {
                        bukkitPlugin.reloadConfig();
                    }
                    player.sendMessage("§eCommand name changes may require a server restart to apply.");
                }
            }
        });
    }
